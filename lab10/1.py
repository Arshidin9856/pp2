...
for u,s in  users:
  if u == user:
    ...
    b=False
if b:
  sql = f"insert into user_score(us,score) values ('{user}',0);"
  cursor.execute(sql)
  conn.commit()
  start=True
...
while running and start:
    ...
    user_name=font1.render(f'User: {user}',True,(255,0,0))
    lev=font1.render(f"Level: {wall.level}", True ,(255,0,0))
    game_over=font.render("GAME_OVER",True,(BLACK))
    screen.blit(text, (20, 20))
    screen.blit(lev, (360, 20))
    screen.blit(user_name, (20, 50))

    if not r:

      sql5 = f"update user_score set score ={score} where user_score.us ='{user}';"
      cursor.execute(sql5)
      conn.commit()      
      cursor.execute(sql1)
      users = cursor.fetchall()
